...
const C: Record<Lang, any> = {
  en: {
    ...
    metaDesc:
      'Just bought a new-build home or condo in Florida? Talk to a licensed agent before you rely on the builder’s bundled insurance. Fast quotes for closing.',
    ...
    sub: 'Builders often bundle you into one insurance option at closing without explaining it. We’ll review your new home’s coverage with you so it’s protected properly from day one.',
    cta: 'Have an agent call me',
    call: `Call ${PHONE_DISPLAY}`,
    whyTitle: 'Why new construction buyers overpay — and what we do about it',
    whyText:
      'A newly built home usually qualifies for real savings: impact windows, a new roof, modern wiring and plumbing, and up-to-code wind mitigation features all lower risk — and should lower your premium too. The problem is most buyers never see those savings, because builders route them straight to one bundled insurance option at the closing table without ever having it explained. As a family-owned local agency, we review your new home’s exact features with you and make sure every wind-mitigation credit you qualify for is actually applied.',
    ...
    cov: [
      ...
      { h: 'Builder’s insurance, explained', p: 'The policy your builder points you to is rarely explained in detail. We’ll walk you through exactly what it covers — and what it doesn’t — before you sign anything.' },
      ...
      { h: 'Bundle with auto & save', p: 'Just moved and need to review your car insurance too? Bundling a new home policy with auto typically unlocks a multi-policy discount on both.' },
    ],
    ...
    faq: [
      ...
      { q: 'My builder already recommended an insurance company — can you still help?', a: 'Absolutely, and it’s worth checking. Builder-recommended policies are convenient but rarely explained in detail. We’ll walk through the same coverage with you so you understand exactly what you’re getting before you commit.' },
  ...
  es: {
    ...
    sub: 'Los constructores suelen incluirlo en una sola opción de seguro al cierre — normalmente no la más barata. Revisamos con usted la cobertura de su nueva casa para que esté bien protegida desde el primer día.',
    ...
    whyText:
      'Una casa recién construida normalmente califica para ahorros reales: ventanas de impacto, techo nuevo, instalaciones eléctricas y de plomería modernas, y características de mitigación de viento al día — todo esto reduce el riesgo y debería reducir su prima también. El problema es que la mayoría de los compradores nunca ven ese ahorro, porque los constructores los dirigen directamente a una sola opción de seguro en la mesa de cierre sin que nadie se lo explique. Como agencia familiar local, revisamos con usted las características exactas de su casa nueva y verificamos que se apliquen todos los créditos de mitigación de viento que le correspondan.',
    ...
    cov: [
      ...
      { h: 'El seguro del constructor, explicado', p: 'La póliza que le recomienda el constructor rara vez se explica en detalle. Le explicamos exactamente qué cubre — y qué no — antes de que firme nada.' },
    ...
    faq: [
      ...
      { q: 'Mi constructor ya me recomendó una aseguradora — ¿aún pueden ayudarme?', a: 'Claro que sí, y vale la pena revisar. Las pólizas recomendadas por el constructor son convenientes pero rara vez se explican en detalle. Revisaremos con usted la misma cobertura para que entienda exactamente qué está recibiendo antes de comprometerse.' },
  ...
  ru: {
    ...
    metaDesc:
      'Купили новый дом или кондо во Флориде? Поговорите с лицензированным агентом, прежде чем соглашаться на вариант от застройщика. Быстрый расчёт для closing.',
    kicker: 'Страховка новостроек · Вся Флорида',
    h1a: 'Страховка нового дома —',
    h1b: 'по цене, справедливой именно для новостройки.',
    sub: 'Застройщики часто предлагают один "готовый" вариант страховки при closing — обычно не самый дешёвый. Мы разберём с вами покрытие вашего нового дома, чтобы он был защищён правильно с первого дня.',
    ...
    cov: [
      ...
      { h: 'Страховка застройщика — разбираем по пунктам', p: 'Полис, который предлагает застройщик, редко объясняют подробно. Разберём с вами, что он покрывает, а что нет, прежде чем вы что-то подпишете.' },
    ...
    faq: [
      ...
      { q: 'Застройщик уже порекомендовал страховую — вы всё равно можете помочь?', a: 'Конечно, и стоит проверить. Полисы, рекомендованные застройщиком, удобны, но их редко подробно объясняют. Мы разберём с вами то же покрытие, чтобы вы точно понимали, что получаете, прежде чем соглашаться.' },
...
export async function generateMetadata({ params }: { params: { lang: string } }) {
  ...
  return pageMetadata({
    lang: params.lang,
    path: '/new-construction-home-insurance-florida',
    title: t.metaTitle,
