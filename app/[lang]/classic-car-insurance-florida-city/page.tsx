...
const C: Record<Lang, any> = {
  en: {
    ...
    faq: [
      ...
      { q: 'Can I drive my classic car every day?', a: 'Generally no — classic car programs are priced for limited, pleasure-driving use. If you drive the car regularly, a standard auto policy (possibly with a stated-value endorsement) may be a better fit, and we can walk you through both.' },
  ...
  ru: {
    ...
    sub: 'Ваша классика — не ежедневный транспорт, и стандартный полис не должен относиться к ней так. Мы поможем оформить покрытие agreed-value, чтобы при полной гибели выплатили реальную стоимость машины.',
    ...
    faq: [
      ...
      { q: 'Могу ли я ездить на классике каждый день?', a: 'Как правило, нет — программы для классики рассчитаны на ограниченное, развлекательное использование. Если вы ездите на машине регулярно, лучше подойдёт обычный автополис (возможно, с эндорсментом stated-value), и мы разберём с вами оба варианта.' },
...
export async function generateMetadata({ params }: { params: { lang: string } }) {
  ...
  return pageMetadata({
    lang: params.lang,
    path: '/classic-car-insurance-florida-city',
    title: t.metaTitle,
...
export default function ClassicCarInsuranceFloridaCity({ params }: { params: { lang: string } }) {
  ...
  return (
    <main>
      ...
      <RelatedCoverage lang={l} current="/classic-car-insurance-florida-city" />
    </main>
  );
}
